...
本项目只负责用户界面、CopilotKit 接入、轻量 API 代理和前端状态展示。不要在这里实现 AgentScope runtime、FastMCP server、Mem0 业务逻辑或 MinIO 持久化逻辑。
...
src/
  app/                  # Next.js App Router 页面、layout、route handlers
    api/copilotkit/     # CopilotKit 到 agent-runtime 的协议边界
...
- `src/app/api/copilotkit/route.ts` 和 `src/app/api/copilotkit/[...slug]/route.ts` 是 CopilotKit runtime HTTP 边界，保持薄。
- `src/lib/copilot-handler.ts` 使用 `CopilotRuntime + HttpAgent` 指向 `agent-runtime /agui/run`。
...
- Server Components 默认用于静态布局；使用 CopilotKit、浏览器 API、动画或事件处理时使用 Client Component。
...
- `agent-runtime` 当前公开 `/agui/run` 和 `/uploads/presign`。
- CopilotKit UI 请求先进入本项目 `/api/copilotkit/*`，再由 `@copilotkit/runtime` 的 `HttpAgent` 转发到 Python AG-UI endpoint。
- 前端 route handler 或 runtime agent 负责补齐本地开发身份 header：`x-tenant-id`、`x-user-id`。
- 协议字段变化时，同步更新 `src/lib/copilot-runtime.ts`、相关测试和 README。
