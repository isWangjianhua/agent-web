      ...
      >
        <AuiIf condition={(s) => !s.thread.capabilities.dictation}>
          <button
            ...
            disabled
            className="flex size-9 items-center justify-center rounded-full bg-[#0d0d0d] text-white opacity-30 dark:bg-white dark:text-black"
          >
            <ArrowUpIcon className="size-6" />
          </button>
        </AuiIf>
    ...
    <AttachmentPrimitive.Root className="group/attachment relative">
      <div className="bg-secondary flex items-center gap-2 overflow-hidden rounded-2xl border dark:bg-white/5">
        <AuiIf condition={(s) => s.attachment.type === "image"}>
          {src ? (
            // assistant-ui attachments can be local object URLs or runtime
            // artifact URLs; next/image cannot optimize those reliably.
