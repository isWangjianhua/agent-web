...
src/app/assistant.tsx     # assistant-ui runtime + ChatGPT clone template 入口
...
assistant-ui 通过 AI SDK transport 请求 `/api/chat`，再由薄 route handler 转发到 `agent-runtime /chat`。UI 层复用 assistant-ui 官方 ChatGPT Clone example，不维护自写 chat/session shell。
